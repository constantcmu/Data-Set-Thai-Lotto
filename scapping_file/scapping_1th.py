        ...
        if columns:
            for col in columns:
                for num in col.find_all('strong'):
                    if len(row['prize_1st']) == 0:  # Only append if list is empty
                        row['prize_1st'].append(num.text)
            return row
    else:
        print(f"Failed to retrieve the page. Status code: {response.status_code}")

columns = ['date', 'prize_1st']
...
while True:
    ...
    if archive.nextPageUrl == "":
        df.to_parquet('lotto_prize_1th.parquet', index=False)
        break
    else:
